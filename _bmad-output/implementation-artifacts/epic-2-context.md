...
Users press a global hotkey from anywhere — including inside a fullscreen game — get a minimal overlay, type a line, hit Enter, and a moment later hear that line spoken in their own cloned voice through a virtual microphone that other apps pick up as input. This is the product's entire reason to exist, delivered end to end: background/tray presence, hotkey configuration, the Prompt Overlay, TTS generation via a Chatterbox sidecar, and playback through a Virtual Microphone. Two distinct technical risks live inside this epic — GPUI/gpui-kit's missing tray+hotkey support, and the Chatterbox sidecar/virtual-mic-driver packaging and control surface — resolved via spike stories early rather than a separate epic, since both belong to one inseparable user outcome.
...
- No network egress outside `voice-me-deps`'s declared GitHub-Releases fetches; the TTS sidecar's IPC is loopback-only. Enforced by a CI check, not review discipline.
...
- IPC framing between `voice-me-tts` and the Python sidecar (stdio JSON-lines vs. local socket) is an implementation detail behind `TtsPort` — either choice is fine architecturally.
...
- Story 2.6 (speech generation) depends on Story 2.4 (Speak Action triggered) and on Epic 1 (an active Reference Voice Sample must exist), and depends on Story 2.5's spike outcome for the sidecar IPC approach and the shared audio buffer type's actual shape.
