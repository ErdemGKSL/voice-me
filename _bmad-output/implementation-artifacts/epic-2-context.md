...
- **Virtual mic:** Linux via a PipeWire/PulseAudio null-sink, no kernel driver and no elevated privileges (to be proven). Windows via `VirtualDrivers/Virtual-Audio-Driver` 25.7.14; its named-pipe/IPC control surface from an unsigned app is unconfirmed — confirm it or pick a documented fallback.
...
- Story 2.9 needs 2.6 (audio exists) and 2.7/2.8 (a working per-OS virtual-mic adapter).
