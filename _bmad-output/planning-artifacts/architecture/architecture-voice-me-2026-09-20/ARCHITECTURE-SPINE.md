...
- **Rule:** GitHub Actions builds Linux and Windows separately (no macOS job in v1); binaries publish to GitHub Releases of this repo. **v1 ships one executable per OS with every backend compiled in, and the user selects the backend inside the app** (product-owner direction, 2026-09-22, reversing the 2026-09-21 download-time-variant direction). Each backend is its own adapter crate behind `TtsPort` — `voice-me-tts-onnx` (CPU/CUDA/WebGPU over one ONNX Runtime) and `voice-me-tts-remote` (DeepInfra, fal.ai) — wired in the composition root. Cargo features exist only to exclude a backend from a *development* build; they never define a shipped product. The asset requirements below are therefore per **backend**, not per download:
  ...
  | Piper | the same core CPU library, no provider libs | one voice, ~63 MB (`model.onnx` + `config.json`, at `<cache>/piper/<key>/`): the default `tr_TR-fahrettin-medium` pinned in code (SHA-256, speaches-ai revision); others chosen in Settings → Piper voices from voice-me's `piper-voices/catalog.json` (SHA-256), `rhasspy/piper-voices` at a pinned revision (MD5) or speaches-ai (LFS SHA-256 / Git blob SHA-1), each checked with its source's digest; not mirrored (per-voice licences) | `voice-me-tts-piper`; CPU execution provider only (added 2026-09-24, catalogs 2026-09-24 addendum) |
  | Edge TTS | none (no ONNX) | none — the `edge-tts` program is installed by the user (`pipx`) | `voice-me-tts-edge`; Linux only (added 2026-09-24) |
...
- **Rule:** exactly two crates may open a socket. `voice-me-deps`, fetching versioned assets from its trusted sources (AD-7), and `voice-me-tts-remote`, calling the speech provider the user selected with the key the user supplied (AD-13). Every other crate is offline: `voice-me-tts-onnx` performs no IPC and opens no socket at all — it reads model files from the cache directory `voice-me-deps` owns — and the `ort` crate must be configured to load a locally provisioned ONNX Runtime (no download-at-build-or-run behaviour). `voice-me-tts-system-linux` and `voice-me-tts-system-windows` open no socket either, and neither do `voice-me-tts-piper` and `voice-me-espeak`. The `edge-tts` child process that `voice-me-tts-edge` runs (Story 3.17) reaches `speech.platform.bing.com`; it is spawned only while Edge TTS is selected and its disclosure is confirmed. voice-me's own code opens no socket for it, so `voice-me-tests`' egress allowlist is unchanged.
...
- **One bounded child-process exception (2026-09-23):** `voice-me-tts-system-linux` runs the system's `espeak-ng` program — resolved by fixed name on `PATH`, text on stdin never argv, no shell, a 10 s deadline — because eSpeak NG is GPL-3.0 and a separate process keeps voice-me's own licence undecided. This rule's "no child process" still binds Chatterbox. Piper (2026-09-24) uses the same program for **phonemization** (`--ipa`, never `--ipa=3`, whose tie characters are not in Piper's phoneme map). The runner lives in exactly one crate, `voice-me-espeak`, which `voice-me-tts-system-linux`, `voice-me-tts-piper` and `voice-me-tts-edge` call; no other crate spawns a process. Piper's VITS graph runs in-process on `ort` like Chatterbox; no Piper engine code (`piper1-gpl`, GPL-3.0; `piper-phonemize`) is linked. Edge TTS (2026-09-24, Story 3.17) adds a second fixed program, `edge-tts` (LGPL-3.0, never linked or shipped): resolved on `PATH` then `~/.local/bin`, text on stdin via `-f -`, audio on stdout, no shell, a 30 s deadline, run through the same one bounded runner.
...
voice-me/
  ...
  crates/
    ...
    voice-me-audio-windows/    # lib: VirtualMicPort adapter - plays to VB-CABLE's "CABLE Input" (Story 2.8)
    ...
    voice-me-tts-edge/          # lib: TtsPort adapter - the edge-tts program via voice-me-espeak's runner (Linux, AD-12 exception), MP3 via symphonia
    voice-me-espeak/            # lib: the one bounded process runner (AD-12 exception) - espeak-ng audio for system-linux, IPA for piper; edge-tts for tts-edge
...
| FR-5 TTS generation (local ONNX, Piper, local system voice, or remote API) | voice-me-tts-onnx, voice-me-tts-piper, voice-me-espeak, voice-me-tts-system-linux, voice-me-tts-system-windows, voice-me-tts-remote, voice-me-tts-edge | AD-1, AD-2, AD-4, AD-5, AD-8, AD-9, AD-10, AD-11, AD-12, AD-13 |
...
- ~~Windows `Virtual-Audio-Driver` control mechanism (named pipe vs. an alternative) — PRD Open Question 2; must resolve before `voice-me-audio-windows` is implemented.~~ **Resolved 2026-09-24 (Story 2.8):** Virtual-Audio-Driver 25.7.14 cannot route speaker to mic (its mic buffer is zero-filled; routing is a paid custom build), so the Windows Virtual Microphone is VB-Audio's signed VB-CABLE. `voice-me-audio-windows` plays to the one output device named `CABLE Input…` through `cpal` (WASAPI), resampling and converting inside the adapter; `voice-me-deps` downloads VB-Audio's pinned driver pack and runs VB's own setup elevated, with the user's consent. VB-CABLE is never mirrored on voice-me's releases or bundled in its package (VB's licence forbids embedding it in another installer).
