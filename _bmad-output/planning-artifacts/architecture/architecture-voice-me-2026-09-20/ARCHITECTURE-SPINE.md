...
- **Whether GPUI/gpui-kit can actually deliver tray + global-hotkey support (PRD Open Question 6, foundational).** Neither provides it upstream; the unofficial "Adabraka GPUI" fork claims to but is unverified. AD-2 already isolates this risk into dedicated adapter crates (`voice-me-tray-*`, `voice-me-hotkey-*`) precisely so the resolution — hand-rolled per-OS shims, the fork, or something else — doesn't ripple into `voice-me-core` or `voice-me-ui` whichever way it lands. Resolve before implementing those crates.
