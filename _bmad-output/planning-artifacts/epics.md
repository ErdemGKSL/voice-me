...
**And** the speech languages are the locales of the voices installed in the cache (stored at `<cache>/piper/<key>/` with a `voice.toml`), with a voice picker when a locale has more than one voice, and a "Manage voices" link to the Piper voices tab
**And** Settings → Piper voices lists, filterably by language and by search, the voices of three catalogs merged in order — voice-me's own `piper-voices/catalog.json` (fetched live; URL, size and SHA-256 per file), `rhasspy/piper-voices`' `voices.json` at a pinned revision (size and MD5), and the `speaches-ai` Hugging Face repositories (revision, LFS SHA-256 of `model.onnx` and Git blob SHA-1 of `config.json`, resolved at download) — the first source winning a duplicate; each row shows name, language, quality, size, source, licence ("see model card" when none is declared) and status, with Download (progress bar), Delete and Use; a catalog that fails is one inline line and the others still show; catalogs are fetched only when the tab is opened or refreshed
**And** the built-in default voice is `tr_TR-fahrettin-medium` (CC0) from `speaches-ai/piper-tr_TR-fahrettin-medium` at a pinned revision, with both files' SHA-256 pinned in code; the voice-me catalog lists the same entry
**And** "No Piper voice installed" (Install downloads fahrettin) or the selected voice missing (Install downloads it when voice-me knows its source) is a dependency row; the ONNX Runtime CPU library row is shared with Chatterbox; `espeak-ng` missing is the manual-steps row from 3.12 — each blocks speech while Piper is selected (Story 3.4)
**And** it is labelled "stock voice", never receives the Reference Voice Sample, opens no socket and needs no disclosure (AD-8)
**And** with no backend in settings, core resolves Piper with fahrettin (`tr_TR`) on Linux (AD-9); an explicit selection is never changed; Settings does not auto-open for a missing Reference Voice Sample while Piper is selected
