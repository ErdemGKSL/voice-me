...
| Backend selector | Settings → Backend, top | Two steps: a Local/Remote choice, then a `Select` of that kind's backends. Local lists engines only, without a device: "Piper — natural, instant", tagged "stock voice", listed first and the first-run default where available; "Chatterbox — your voice"; and "System voice — instant" — eSpeak NG on Linux, Windows voices on Windows, tagged "stock voice". Picking Piper or Chatterbox shows a second `Select` under it, **Device**: "CPU", "CUDA", "WebGPU" on the bundled runtime (CUDA and WebGPU only once the bundled runtime has every provider), plus, for Chatterbox, each added runtime's targets ("CUDA — libonnxruntime.so"); Piper runs on the bundled runtime only. Switching engine keeps the device when the new engine offers it, otherwise CPU. The System voice and remote backends show no device `Select`. Remote: DeepInfra, fal.ai, Azure, and "Edge TTS — free, online" — Azure and Edge TTS tagged "stock voice"; Edge TTS has no key field and is always selectable, installed or not. Below it, only the selected backend's options. Beneath all, read-only, what the selection **actually acquired** at session build, which is not always what was selected (AD-9). |
